...
- スマホでは比較表をカード表示に固定し、横長テーブルを非表示にしました。
- スマホでは比較一覧を商品追加フォームより先に表示します。
- 画面下に「比較 / 追加 / 管理」のショートカットナビを追加しました。
- PC版と同じ青系アクセントカラーをスマホにも反映しました。
- 入力欄、CPU/GPUの段階選択、自作PCパーツ入力をスマホ幅で1列表示に調整しました。
- CSS/JSにバージョン付きクエリを付け、GitHub Pages上で古いキャッシュが残りにくいようにしました。
